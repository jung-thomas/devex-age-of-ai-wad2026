---
theme: '@jungsap/slidev-theme-sap'
title: Evolving the Developer Experience in the Age of AI
info: |
  WeAreDevelopers World Congress 2026 — Friday, July 10, Mainstage, Berlin.
  Thomas Jung & Shilpa Shankar, SAP Developer Advocacy.
---

---
layout: cover
variant: a
title: Evolving the Developer Experience in the Age of AI
presenter: thomas-jung
event: WeAreDevelopers World Congress 2026
date: 2026-07-10
classification: PUBLIC
partnerLogo: /partners/wad-logo.svg
---

---
layout: title-text
title: Your speakers
---

<Bio presenter="thomas-jung" />

<Bio presenter="shilpa-shankar" />

---
layout: agenda
title: Agenda
items:
  - title: The DevEx gap in the AI era
    description: Why generic AI interfaces aren't enough for enterprise developers
  - title: Embedding intelligence in the workflow
    description: AI where developers already work — not in a separate tab
  - title: Rebuilding learning with generative AI
    description: How SAP retooled its developer learning infrastructure
  - title: Listening at scale
    description: Turning developer feedback into tooling decisions
  - title: Education + community = adoption
    description: Combining automated guidance with human community support
  - title: What's next
    description: Open questions and where this is heading
---

---
layout: divider
title: The DevEx gap in the AI era
---

---
layout: title-text
title: Generic AI isn't enough
---

Enterprise developers don't need *another* chat window. They need
intelligence that understands their context: the codebase, the
data model, the deployment target, the compliance constraints.

<!--
Speaker notes: lead with the friction story — every dev advocate
has heard "ChatGPT is great but it doesn't know our system."
-->

---
layout: divider
title: Embedding intelligence in the workflow
---

---
layout: title-text
title: Where developers already are
---

The win isn't a better assistant — it's an assistant that shows
up at the right moment inside the IDE, the CLI, the docs, the
pipeline. **Right tool, right context, right time.**

---
layout: divider
title: Rebuilding learning with generative AI
---

---
layout: title-text
title: Tutorials that adapt to you
---

Static "Hello World" tutorials don't scale across thousands of
products and personas. Generative AI lets us produce learning
paths that respond to what each developer is actually building.

---
layout: divider
title: Listening at scale
---

---
layout: title-text
title: Feedback as a first-class signal
---

When 100,000+ developers use your platform, surveys aren't the
fastest signal. Instrumented IDE telemetry + community sentiment
analysis surface issues before the support backlog does.

---
layout: divider
title: Education + community = adoption
---

---
layout: title-text
title: Humans + AI, not humans or AI
---

Automated guidance gets developers unstuck for the first 80%.
Community — peers, advocates, champions — handles the long tail.
Neither alone scales; together they cover the gap.

---
layout: divider
title: What's next
---

---
layout: title-text
title: Open questions
---

- How do we measure "developer joy," not just "time to first commit"?
- What's the right boundary between AI suggestions and developer agency?
- How does DevRel evolve when AI handles the easy questions?

---
layout: q-and-a
title: Questions?
---

<Speaker :presenters="['thomas-jung', 'shilpa-shankar']" />

Find us after the talk, or via the socials on our bios.

---
layout: thank-you
title: Thank you
---
